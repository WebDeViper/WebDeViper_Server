...
// 그룹 프로필 이미지 업로드된 파일을 저장할 디렉터리 및 파일명 설정
const groupImgStorage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, path.join(__dirname, '../../static/groupImg')); // 파일을 저장할 디렉터리
  },
  filename: function (req, file, cb) {
    const ext = path.extname(file.originalname);
    const sanitizedFilename = sanitizeFilename(path.basename(file.originalname, ext));
...
// 그룹 이미지 파일 필터 함수 정의 (이미지 파일만 허용)
const groupImgFileFilter = (req, file, cb) => {
  const allowedMimeTypes = ['image/tiff', 'image/png', 'image/jpg', 'image/jpeg', 'image/png', 'image/gif']; // 허용할 이미지 MIME 타입 목록
...
// 프로필 이미지 파일 업로드를 처리할 Multer 인스턴스 생성
const groupImgUploader = multer({
  storage: groupImgStorage,
  fileFilter: groupImgFileFilter, // 파일 필터 적용
  limits: { fileSize: 5 * 1024 * 1024 }, // 5MB
});
